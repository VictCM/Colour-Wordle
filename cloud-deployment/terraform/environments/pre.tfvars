...
# ------------------------------------------------------
sftp_conf = {
  dns_subdomain_hostname = "sftp"
  sftp_users = {
    backup = "sdwan-fortinet-pre-app-backup-137062172019/backup" ### [user_name,s3name_home_dir_path]
  }
}
...
forti_instances = { ### some cases if values are "" or null then it get forti_instances_default_conf values
  fortianalyzer_01 = {
    ...
    secure_instance_id         = "46566c416064f8dfd717" ###  ["" | null | secure_id] if empty | null use random_id resource generated. Other case use secure_id
    ...
    snapshot_tags_selector     = null
    sg_rules                   = null
    serial_number              = "22000931"
  }
  fortimanager_01 = {
    instance_type              = "fortimanager"
    secure_instance_id         = "e83b03a8dfd2aabf19e4"
    network_private_ip_hostnum = 4
    permanent_eip              = true
    compute_instance_type      = null
    compute_ami                = null
...
forti_instances_default_conf = {
  fortianalyzer = { ###  [ "fortianalyzer" | "fortimanager" | "fortiportal"]
    ...
    sg_rules = {            ###  [i/o "ingress|egress" , cidr_block "cidr|any" , from_port|icmp_type "port|type" , to_port|icmp_code "port|code", proto "tcp|udp|icmp|-1|all" , desc]
      ...
      "8-icmp-eg"   = ["egress", "any", 8, 0, "icmp", "ICMP support"],
      "22-tcp-eg"   = ["egress", "any", 22, 22, "tcp", "SSH port to reach Backup instance"],
      "53-udp-eg"   = ["egress", "any", 53, 53, "udp", "DNS lookup"],
      "123-udp-eg"  = ["egress", "any", 123, 123, "udp", "NTP sync"],
      "443-tcp-eg"  = ["egress", "any", 443, 443, "tcp", "Forticloud"],
      "541-tcp-eg"  = ["egress", "any", 541, 541, "tcp", "Register FGT devices to FM or FA for configuration management"],
      "542-tcp-eg"  = ["egress", "any", 542, 542, "tcp", "FGTs mgmt & register devices"],
      "4443-tcp-eg" = ["egress", "any", 4443, 4443, "tcp", "Fortinet registry for mgmt ext. apps, such as FortiWLM ME"],
      "8880-tcp-eg" = ["egress", "any", 8880, 8880, "tcp", "Turn closed network mode logic on/off"]
    }
    serial_number  = ""
  },
  fortimanager = {
    ...
    sg_rules = {
      ...
      "123-udp-eg"  = ["egress", "any", 123, 123, "udp", "NTP sync"],
      "443-tcp-eg"  = ["egress", "any", 443, 443, "tcp", "FortiCloud"],
      "514-udp-eg"  = ["egress", "any", 514, 514, "udp", "MNG to FAZ"],
      "514-tcp-eg"  = ["egress", "any", 514, 514, "tcp", "MNG to FAZ"],
      "541-tcp-eg"  = ["egress", "any", 541, 541, "tcp", "Register FGT devices to FM or FA for configuration management"],
      "542-tcp-eg"  = ["egress", "any", 542, 542, "tcp", "FGTs mgmt and register devices"],
      "4443-tcp-eg" = ["egress", "any", 4443, 4443, "tcp", "Fortinet registry for mgmt ext. apps, such as FortiWLM ME"],
      "8880-tcp-eg" = ["egress", "any", 8880, 8880, "tcp", "Turn closed network mode logic on/off"]
    }
    serial_number  = ""
  ...
  fortiportal = {
    ...
    base_storage_conf = {
      ...
      data = [80, "gp3", true, true, true, "storage-data-sdb", "/dev/sdb"] ### AMI mandatory volume as a minimum 80GB
    }
    extra_storage_conf = {}
    sg_rules = {
      "8-icmp-in"  = ["ingress", "10.201.0.95/32", 8, 0, "icmp", "ICMP SNMP monitoring woku"],
      ...
      "161-udp-in" = ["ingress", "10.201.0.95/32", 161, 161, "udp", "Woku integration"],
      "443-tcp-in" = ["ingress", "any", 443, 443, "tcp", "HTTPS admin access"],
      "any-tcp-eg" = ["egress", "any", 0, 0, "-1", "All egress open"]
    }
    serial_number  = ""
  }
}
...
fortiportal_db_conf = {
  instance_conf = {
    ...
    security_access_conf = {
      public_access   = false
      allowed_sources = ["any"] ### fortiportals always permitted  ### [cidr | "vpc" | "any"]*  Ex: ["vpc", "192.168.0.0/28"]   ["vpc", "any", "135.2.3.2/32"]
    }
  }
  engine_conf             = ["mariadb", "10.4.21"]                                  ### [ engine, engine_version]
  maintenance_backup_conf = ["Sun:00:00-Sun:03:00", "05:00-07:00", 7, false, true] ### [maintenance_window, backup_window, backup retention, db_instance_deletion_protection, delete_backups_after_db_termination]
